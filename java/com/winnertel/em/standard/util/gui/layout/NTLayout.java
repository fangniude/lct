...
public class NTLayout implements LayoutManager, java.io.Serializable {
     ...
     */
    public NTLayout(int rows, int cols) {
     ...
     */
    public NTLayout(int rows, int cols, int hgap, int vgap) {
    ...
    public int getIndex(int row, int col, int ncomponents) {
        int nrows = rows;
        int ncols = cols;
     ...
     * this grid layout.
     * <p/>
     * Every column has its prefered width
     * column scales are justified on its preferred size.
     * <p/>
     * Every row has its prefered height.
     * row scales are justified on its prefered size
     *
    ...
    public void layoutContainer(Container parent) {
        ...
        Insets insets = parent.getInsets();
        int x, y = topBorder + insets.top;
        allocateSuitableSizes(parent);
        for (int i = 0; i < rows; i++) {
            x = leftBorder + insets.left;
            for (int j = 0; j < cols; j++) {
    ...
    /**
     * Update out the Suitable sizes for each of the grid cells
     * using the specified scale values for rows and columns.
     * if there are spacer ,them pad the rest space on average.
     *
     * @param parent the container to be laid out.
     */

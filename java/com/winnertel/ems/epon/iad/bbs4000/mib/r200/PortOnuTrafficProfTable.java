...
public class PortOnuTrafficProfTable extends SnmpMibBean {
  ...
  public Vector retrieveAll() throws MibBeanException {
    ...
    if(getIndex(0) != null) {
      Vector list = new Vector(128);

      for(int i = 1; i <= 4; i++) {
        for(int j = 1; j <= 32; j++) {
          PortOnuTrafficProfTable bean = new PortOnuTrafficProfTable(fSnmpProxy);

          bean.setUtsDot3OnuTProfModuleId((Integer)getIndex(0));
          bean.setUtsDot3OnuTProfDeviceId(1);
          bean.setUtsDot3OnuTProfPortId(i);
          bean.setUtsDot3OnuTProfLogicalPortId(j);
          bean.retrieve();

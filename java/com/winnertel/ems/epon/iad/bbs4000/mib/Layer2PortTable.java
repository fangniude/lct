 ...
 */
public class Layer2PortTable extends SnmpMibBean
{
	...
	public Layer2PortTable(ISnmpProxy aSnmpProxy)
	{
		super(aSnmpProxy);
		init();
	}
...
//		prepareRead(super.getProperty(utsEthIfExtIfIndex));
		...
		Vector result = loadAll(new int[]
		{ 1 });

    int i = 0;
    while(i < result.size())
    {
      Layer2PortTable ifBean = (Layer2PortTable)result.get(i);
      if(ifBean.getUtsEthIfExtSwitchMode().intValue() != 1) //1 - layer2
      {
        result.remove(i);
      }
      else

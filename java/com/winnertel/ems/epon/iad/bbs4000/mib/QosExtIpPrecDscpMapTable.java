...
public class QosExtIpPrecDscpMapTable extends SnmpMibBean
{
    ...
    public boolean delete() throws MibBeanException{
        Vector v = new QosExtIpPrecDscpMapTable(fSnmpProxy).retrieveAll();
        if(v!=null){
            for(int i=0;i<v.size();i++){
                QosExtIpPrecDscpMapTable one = (QosExtIpPrecDscpMapTable)v.get(i);
                one.setUtsMLSQosExtIpPrecDscpMapDscpVal(new Integer( i*8) );

...
public class InterfaceTableMBean extends SnmpMibBean {
    ...
    */
    ...
    public Vector retrieveAll() throws MibBeanException {
        ...
        if (getIfIndex() != null) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                bean.retrieve();
                bean.setIfDescr(getIfDescr());
                ifIndex += 1024;

                //filter the port on module. XGM has only one port. the others port in the vector has null attribute.
                if(bean.getIfOperStatus() != null) {
                    v.add(bean);
                }
            }
        } else {
    ...
    public boolean checkIfIndex(Integer i) {
        //refer to 'com.winnertel.ems.epon.iad.bbs4000.ConfigUtility.formatIfIndex()'
        int highBits = i >>> 27;
        if (highBits == 0) {

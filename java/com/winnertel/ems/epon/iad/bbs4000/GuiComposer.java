...
import com.winnertel.ems.epon.iad.bbs4000.gui.shelfview.ShelfViewPane;
...
public final class GuiComposer implements IGuiComposer {
    ...
    public void composeTabbedPane(String aName, JTabbedPane aTabbedPane) {
        ...
        if (aTabbedPane.getSelectedComponent() != null)
            previous = aTabbedPane.getSelectedComponent().getName();
        ...
        while (it.hasNext()) {
            ...
            if (panel != null) {
                ...
                if (helpId != null && helpId.length() > 0) {
                    panel.registerKeyboardAction(new HelpAction(fApplication, helpId),
        ...
        for (int i = 0; i < aTabbedPane.getTabCount(); i++) {
            if (aTabbedPane.getComponentAt(i).getName().equals(previous)) {
                aTabbedPane.setSelectedIndex(i);
            }
        }
    }
    ...
    public UPanel composeSnmpTablePane(String aName) {
        ...
        if (ePanel == null) {
            fLogger.error("[GuiComposer.composeSnmpTablePane()] Fail to get Panel class for " + aName, new RuntimeException());
            return null;
        }
        ...
        */
        ...
        // add by Li Pengpeng for supporting other transport layer protocol, such as SFTP
...
//                    String neName = ClientUtility.getNeNameByNeIp(fApplication.getSnmpProxy().getTargetHost());
    ...
    private Object[] generateIndex(String panel) {
        ...
        if (panel.equals("QosExtCosQueueSchedTable_Panel") || panel.equals("QosExtCosQueueSchedTable_Panel_xgm")) {
            result = new String[2];
            result[0] = "" + (slotId - 1) * 4;
            result[1] = "7";
            ...
            result[1] = "7";
                ...
                || panel.equals("QosExtDscpTrustTable_Panel_xgm")
                || panel.equals("SwitchPortExtTable_Panel")
                || panel.equals("SwitchPortExtTable_Panel_xgm")) {
            result = new String[1];
            result[0] = "" + (slotId - 1) * 4;
        } else if (panel.equals("LgQosExtDscpTrustTable_Panel")
                || panel.equals("SwitchPortExtTable_Panel_lg")) {
            result = new String[1];
            result[0] = "48";
        } else if (panel.equals("Dot3OnuTable_Panel")
                || panel.equals("Dot3DbaSlaTable_Panel")
                || panel.equals("Dot3OnuOamTable_Panel")
                || panel.equals("Dot3boundOnuTable_Panel")
                || panel.equals("Dot3OnuRTTPanel")
                || panel.equals("Dot3OnuPingPanel")
                || panel.equals("Dot3OnuLinktestPanel")
                || panel.equals("Dot3OnuLoopbackPanel")) {
            /*The port ifIndex is the previous row data.*/
            int ifIndex = ConfigUtility.generateIfIndex((slotId) + "/1"); //port if index
            result = new String[1];
            result[0] = "" + ifIndex;
        } else if (panel.equals("Dot3UnboundOnuTable_Panel")) {
            result = new String[3];
            result[0] = "" + slotId;
            result[1] = "0";
            result[2] = "0";
            ...
            result[1] = 0;
            result[2] = 0;
        } else if (panel.equals("Dot3OltTable_Panel") || panel.equals("Dot3DbaOltTable_Panel")
                || panel.equals("EthVlanTransparentTable")) {
            int ifIndex = ConfigUtility.generateIfIndex((slotId) + "/1"); //port if index
            result = new String[1];
        ...
        } else if (panel.equals("IntfRateControlTable_Panel")
                || panel.equals("IntfRateControlTable_Panel_gem")
                || panel.equals("IntfRateControlTable_Panel_xgm")) {
            result = new String[2];
            result[0] = "" + (slotId - 1);
            result[1] = "4";
            ...
            result[0] = (ifIndex - 29696 + ne.getPonSplitRatio());
                ...
                || panel.equals("Interface_table_lg")
                || panel.equals("Ethernet_Performance")
                || panel.equals("OLT_Performance")
                || panel.equals("ONU_Performance")) {
            int ifIndex = ConfigUtility.generateIfIndex((slotId) + "/1");
            result = new Integer[1];
            result[0] = (ifIndex - 29664);
        } else if (panel.equals("Interface_table_gem")
                || panel.equals("Interface_table_xgm")
                || panel.equals("Ethernet_Performance_xgm")) {
            int ifIndex = ConfigUtility.generateIfIndex((slotId) + "/1");
            result = new Integer[1];
            result[0] = (ifIndex - 29632);
            ...
            result[0] = slotId - 1;
            result[1] = 1;
        } else if (panel.equals("Dot3Olt2PortTable_Panel") || panel.equals("Dot3Olt2PortDbaTable_Panel")
                || panel.equals("OnuDiagnosisTableDetail_Panel") || panel.equals("OnuLoopbackTestTable_Panel")
            ...
            result[3] = onuId;
            result[4] = 0;
        } else if (panel.equals("Dot3Olt2VlanStackingTable_Panel")) {
            result = new Integer[5];
            result[0] = slotId;
            result[1] = 1;
            result[2] = portId;
            ...
            result[4] = 0;
                ...
                || "Dot3Onu2CtcConfigure_Panel".equals(panel)
                || "MulticastConfigure_Panel".equals(panel)) {
            result = new Integer[4];
            result[0] = slotId;
            result[1] = 1;
            result[2] = portId;
            result[3] = onuId - 1;
        } else if (panel.equals("Dot3Onu2CtcValidOnuMacTable_Panel") || panel.equals("OnuPowerMeasure_Panel")
                || panel.equals("Dot3OnuRTT_Panel") || panel.equals("PortOnuBatchTrafficProfTable_Panel")
                || panel.equals("UtsDot3Onu2OPTDTable_Panel")) {
            result = new Integer[4];
            result[0] = slotId;
            result[1] = 1;
            result[2] = 1;
            ...
            result[5] = 32;
            result[6] = 120;
        } else if (panel.equals("SelectiveQinQMBeanPanel_Panel")) {
    ...
    private int getRowsToFetch(String panel) {
        ...
        if (panel.equals("Dot3Olt2PortTable_Panel") || panel.equals("Dot3Olt2PortDbaTable_Panel")
                || panel.equals("PonIf2ExtStatTable_OLT_Panel") || panel.equals("SwitchPortExtTable_Panel")
                || panel.equals("OnuDiagnosisTableDetail_Panel") || panel.equals("OnuLoopbackTestTable_Panel")
                || panel.equals("EthVlanTransparentTable")) {
            return 4;
        }
        ...
        if (panel.equals("Dot3Onu2StaticFDBTable_Panel") || panel.equals("Dot3Onu2FDBTable_Panel")
                || panel.equals("Dot3Onu2CtcTable_Panel")
                || panel.equals("Dot3Onu2CtcDbaTable_Panel")
                || panel.equals("Dot3Onu2QoSTable_Panel")
                || panel.equals("Dot3Onu2TrafficProfTable_Panel")) {
            return 1;
        }
    ...
    private String getIndexPrefix(String panel) {
        ...
        if (panel.equals("Dot3Onu2CtcValidOnuMacTable_Panel") || panel.equals("PonIf2ExtStatTable_ONU_Panel")
                || panel.equals("OnuCtcMulticastOnlineGroupStatTable_Panel") || panel.equals("UtsDot3Onu2OPTDTable_Panel")) {
            prefix = sSlotId + ".1";
        } else if (/*panel.equals("EthSmartQinQTable_Panel") || */panel.equals("EthVlanMappingTable_Panel")
            ...
            prefix = sSlotId + ".";
                ...
                || "UNIVlanStackingMBean_Panel".equals(panel)
                || "Dot3Onu2CtcEtherPortRateTable_Panel".equals(panel)
                || "SelectiveQinQMBeanPanel_Panel".equals(panel)
                ) {
    ...
    private boolean checkVersion(String version) {
        if (version != null) {
            if (version.contains("[") && version.contains("]")) {
                ...
                try {
                    String tVersion = SnmpUtility.getNeTotalVersion(neType,
                            ipAddress, port, getCommunity);
                    if (tVersion != null) {
                        if (tVersion.compareTo(split[0]) >= 0
                                && (split.length < 2 || split[1] != null
                                && split[1].trim().length() != 0
                                && tVersion.compareTo(split[1]) < 0))
                            return true;
                    }
                } catch (Exception e) {
                    e.printStackTrace();

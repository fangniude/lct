...
//import com.winnertel.ems.epon.iad.bbs4000.mib.r300.OltPonLinkCfgTable;
...
//      }
  ...
  private boolean initConstruct()
  {
    ...
    {
      long t1 = System.currentTimeMillis();
      URL url = ShelfView.class.getResource("/com/winnertel/ems/epon/iad/bbs4000/gui/shelfview/resource/background.jpg");
...
//          {
//            String strOid = (String)trapVec.elementAt(i);
//
//            if(pdu.getVariable(1).equals(new UTSnmpOID(strOid)))
//            {
//              updateOpeState();
//            }
//          }
  ...
  JPopupMenu createContextualPopup(JComponent component, Element element, Point2D d)
  {
    ...
    if(element instanceof Card)
    {
      ...
      {
/* fix for MR: NMS72427, remove "lock / unlock Pon board" context menu 
		String neVersion = theApplication.getActiveDeviceManager().getVersion();
		if (!(selectedObjectType.startsWith("EPM") && neVersion.startsWith("3.1"))) {
...
//      ex.printStackTrace();
  ...
  private void executeAction(String action) throws Exception
  {
    ...
    else if(type.equals("card"))
    {
      ...
      if(slotId.indexOf("PWR") != -1)
      {
        if(action.equals("Refresh"))
        {
          try
          {
            bean = new ChassisConfTable(theApplication.getSnmpProxy());
      ...
      {
        bean = new BBS4000CardMibBean(theApplication.getSnmpProxy());
        ((BBS4000CardMibBean)bean).setUtsEponModuleBoardPhyId(new Integer(property.getProperty("slotId")));
        if(action.equals("Lock_Card"))
        {
          ((BBS4000CardMibBean)bean).setUtsEponModuleAdminState(new Integer(1));  //1-locked
        }
        else if(action.equals("Unlock_Card"))
        {
          ((BBS4000CardMibBean)bean).setUtsEponModuleAdminState(new Integer(2));  //2-unlocked
        }
        else if(action.equals("Reset_Card"))
        {
          ((BBS4000CardMibBean)bean).setUtsEponModuleAdminState(new Integer(8));  //8-reset
      }
      ...
      // NMS00060620
      String warningTitle = theApplication.getActiveDeviceManager().getGuiComposer().getString("Risky Operation Prompt");
      String warningMessage = theApplication.getActiveDeviceManager().getGuiComposer().getString("Are you sure to do this configuration?");
      
      int confirmResult = JOptionPane.showConfirmDialog(theApplication.getTopMostFrame(),
      		warningMessage,
      		warningTitle,
    ...
    else if(type.equals("port"))
    {
      ...
      else if(portType.equals("GPM"))
      {
        bean = new OltPonLinkCfgTable(theApplication.getSnmpProxy());
        String[] split = portId.split("/");
        ((OltPonLinkCfgTable)bean).setOltPonLinkCfgSlotNo(new Integer(split[0]));
        ((OltPonLinkCfgTable)bean).setOltPonLinkCfgPortNo(new Integer(split[1]));
        if(action.equals("Lock_Port"))
        {
          ((OltPonLinkCfgTable)bean).setOltPonLinkCfgAdministrativeState(new Integer(2));  //2-down
        }
        else if(action.equals("Unlock_Port"))
        {
          ((OltPonLinkCfgTable)bean).setOltPonLinkCfgAdministrativeState(new Integer(1));  //1-up
        }
        else
        {
          throw new OperationCanceledException("Not Supported Command: " + action);
      ...
      else
      {
        ...
        ((Dot3Olt2PortTable)bean).setUtsDot3OltDeviceId(1);
      ...
      if(b.equals(Boolean.TRUE))
      {
        int card = ((Integer)bean.getIndex(0)).intValue();

        //the card is not CSM card.
        if(card != 13 && card != 14)
        {
          shelfViewData.update(bean);
        }
...
*/
    ...
    else if(type.equals(".1.3.6.1.4.1.41355.1.3.10.200.6.3.1.0.16")) //utsFanNormal
    {
      ChassisConfTable bean = new ChassisConfTable(theApplication.getSnmpProxy());
      int position = Integer.parseInt(pdu.getVariableBinding(9).getVariable().getVarObject().toString());
      if(position == 1)
      {
        bean.setUtsBBSChassisLeftFanTrayStat(new Integer(2));
      }
      else if(position == 2)
      {
        bean.setUtsBBSChassisMiddleFanTrayStat(new Integer(2));
      }
      else if(position == 3)
      {
        bean.setUtsBBSChassisRightFanTrayStat(new Integer(2));
      }
      else
      {
        return;
    ...
    else if(type.equals(".1.3.6.1.4.1.41355.1.3.10.200.6.3.1.0.18")) //utsFanAbNormal
    {
      ChassisConfTable bean = new ChassisConfTable(theApplication.getSnmpProxy());
      int position = Integer.parseInt(pdu.getVariableBinding(9).getVariable().getVarObject().toString());
      if(position == 1)
      {
        bean.setUtsBBSChassisLeftFanTrayStat(new Integer(1));
      }
      else if(position == 2)
      {
        bean.setUtsBBSChassisMiddleFanTrayStat(new Integer(1));
      }
      else if(position == 3)
      {
        bean.setUtsBBSChassisRightFanTrayStat(new Integer(1));
      }
      else
      {
        return;
    ...
    {
      BBS4000CardMibBean bean = new BBS4000CardMibBean(theApplication.getSnmpProxy());
      ...
      bean.setUtsEponModulePhyPresenceStat(new Integer(1)); //installed
    }
    else if(type.equals(".1.3.6.1.4.1.41355.1.3.10.200.6.3.1.0.38")) //utsBBS4000CardRemoveAlarm
    {
      ...
      bean.setUtsEponModuleBoardPhyId(new Integer(pdu.getVariableBinding(9).getVariable().getVarObject().toString()));
      bean.setUtsEponModuleBoardType(new Integer(pdu.getVariableBinding(10).getVariable().getVarObject().toString()));
      bean.setUtsEponModulePhyPresenceStat(new Integer(2)); //uninstalled
    }
    else
    {
    }

...
public class GEMCardGenerator
{
  IApplication application = null;
  ModuleType moduleType = new ModuleType();

  public GEMCardGenerator(IApplication application)
  {
...
//      throw new IllegalArgumentException("Unsupport Card Type: " + bean.getUtsEponModuleBoardType());
    ...
    card.setTypeInfo(new TypeInfo("GEM"));
    ...
    if(bean.getUtsEponModulePhyPresenceStat().intValue() == 1) //installed
    {
      ...
      {
        card.setImage(ILogResourceFactory.getCardImage("GEM04B"));
      }
      else if(type == 12)
      {
        card.setImage(ILogResourceFactory.getCardImage("GEM04C"));
      }
      else
      {
        card.setImage(ILogResourceFactory.getCardImage("GEM04A"));
...
//      card.setItem(lnkLed, new IlvPoint(33, 212-(i-1)*9));
  ...
  public void update(UDataBox dataSource, BBS4000CardMibBean bean)
  {
    ...
    if(card.getTypeInfo().getName().startsWith("GEM"))
    {
      if(bean.getUtsEponModulePhyPresenceStat().intValue() == 1) // installed
      {
        int type = bean.getUtsEponModuleBoardType().intValue();

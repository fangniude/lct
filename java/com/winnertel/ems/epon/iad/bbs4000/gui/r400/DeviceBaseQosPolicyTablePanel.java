...
import com.winnertel.ems.epon.iad.bbs4000.mib.r400.DeviceBaseQosPolicyTable;
...
public class DeviceBaseQosPolicyTablePanel extends UPanel {
    ...
    private final String baseQosPolicyMode = fStringMap.getString("deviceBaseQosPolicyMode") + " : ";
    private final String baseQosPolicyWeightOctet = fStringMap.getString("deviceBaseQosPolicyWeightOctet") + " : ";
    private final String baseQosPolicySpBandwidthRange = fStringMap.getString("deviceBaseQosPolicySpBandwidthRange") + " : ";

    public DeviceBaseQosPolicyTablePanel(IApplication app) {
        super(app);
        init();
    }
    ...
    public void initGui() {
        ...
        NTLayout layout1 = new NTLayout(2, 9, NTLayout.FILL, NTLayout.CENTER, 5, 5);
        ...
        NTLayout layout2 = new NTLayout(2, 9, NTLayout.FILL, NTLayout.CENTER, 5, 5);
        ...
        tfBaseQosPolicyMode.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                int mode = tfBaseQosPolicyMode.getSelectedIndex() + 1;
                initWeightPanel(mode);
                initBandwidthRangePanel(mode);
            }
        });
...
//        setPreferredSize(new Dimension(500, 550));
    ...
    public void initBandwidthRangePanel(int mode) {
        if (mode == 2) { //wrr
            for (int i = 0; i < tfBaseQosPolicySpBandwidthRange.length; i++) {
                tfBaseQosPolicySpBandwidthRange[i].setValue(0);
                tfBaseQosPolicySpBandwidthRange[i].setEditable(false);
            }
        } else {
            for (int i = 0; i < tfBaseQosPolicySpBandwidthRange.length; i++) {
                tfBaseQosPolicySpBandwidthRange[i].setValue(0);
    ...
    public boolean validateFrom() {
        ...
        boolean ifwrr=baseQosPolicyModeVList[tfBaseQosPolicyMode.getSelectedIndex()]==2;
        boolean result = (n == 0 || n == 100);
        if(ifwrr) result= (n == 1 || n == 100);
        if (!result)
    ...
    public void refresh() {
        DeviceBaseQosPolicyTable mbean = (DeviceBaseQosPolicyTable) getModel();
        if (mbean == null)
            return;

        tfBaseQosPolicyMode.setSelectedIndex(getIndexFromValue(baseQosPolicyModeVList, mbean.getDeviceBaseQosPolicyMode()));
        //initWeightPanel(mbean.getDeviceBaseQosPolicyMode()); //manually trigger 'addItemListener'
        setBaseQosPolicyWeightOctet(mbean.getDeviceBaseQosPolicyMode(), mbean.getDeviceBaseQosPolicyWeightOctet());
        setBaseQosPolicySpBandwidthRange(mbean.getDeviceBaseQosPolicySpBandwidthRange());
    }
    ...
    public void updateModel() {
        ...
        mbean.setDeviceBaseQosPolicyMode(new Integer(baseQosPolicyModeVList[tfBaseQosPolicyMode.getSelectedIndex()]));
        mbean.setDeviceBaseQosPolicyWeightOctet(getBaseQosPolicyWeightOctet(mbean.getDeviceBaseQosPolicyMode()));
        mbean.setDeviceBaseQosPolicySpBandwidthRange(getBaseQosPolicySpBandwidthRange(mbean.getDeviceBaseQosPolicyMode()));
    }
    ...
    private byte[] getBaseQosPolicySpBandwidthRange(int mode) {
        if (mode == 2) { //wrr
            byte[] b = new byte[tfBaseQosPolicyWeightOctet.length];
            //for (int i = 0; (i < tfBaseQosPolicyWeightOctet.length); i++)
            //    b[i] = 0;

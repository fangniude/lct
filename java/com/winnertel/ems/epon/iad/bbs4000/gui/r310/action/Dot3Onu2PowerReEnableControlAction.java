...
public class Dot3Onu2PowerReEnableControlAction extends SnmpAction {

	public Dot3Onu2PowerReEnableControlAction(IApplication anApplication) {
		super(anApplication);
	}
	...
	public void executeAction(ActionEvent e) throws OperationFailedException,
		...
		Dot3Onu2OperationTable fBean = (Dot3Onu2OperationTable)fPanel.getModel();
		if(fBean == null)
			fBean = new Dot3Onu2OperationTable(this.fApplication.getSnmpProxy());
		...
		fBean.setUtsDot3Onu2PowerControlActions(new Integer(0));
		fBean.setUtsDot3Onu2PowerControlMode(bean.getUtsDot3Onu2CtcRegisterStatus() == 1 ? 1 : 2); //����ONUȱʡ�õ���LLID,�����ù㲥LLID
//		fBean.setUtsDot3Onu2PowerControlOnuId(bean.getUtsDot3Onu2CtcRegisterStatus() == 1  ? null : 
//			getMacAddress(bean.getUtsDot3Onu2CtcOnuMacAddr()));  //Valid only on boardcast LLID mode.�ڵ���ʱ�ҵ�

...
public class StopDiagnosisAction extends SnmpAction{

	public StopDiagnosisAction(IApplication anApplication) {
		super(anApplication);
	}
	...
	public void executeAction(ActionEvent arg0) throws Exception {
		...
		if (!( status < 0 && status < 100 )) {
			MessageDialog.showErrorMessageDialog( fApplication, 
					fApplication.getActiveDeviceManager().getGuiComposer().getString("error_not_starting"));
			return;
		}
		
		if ( MessageDialog.showConfirmDialog( fApplication, 
				fApplication.getActiveDeviceManager().getGuiComposer().getString( "sure_to_continue_stop_diagnosis" )) 
				==	JOptionPane.NO_OPTION ){
			return;
		}
		
		diagBean.setUtsDot3OltPortDiagnosisAction( PortDiagnosisMBean.STOP );

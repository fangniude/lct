...
//    category_map.clear();
    ...
    try
    {
      ...
      l2PortList = filterXgmPort( l2PortList, cardList );
  ...
  private Vector getGsmCardList() throws Exception
  {
    ...
    while(i < v.size())
    {
      if(!moduleType.isGEMModule(((BBS4000CardMibBean)v.get(i)).getUtsEponModuleBoardType().intValue()) &&
         !moduleType.isXGMModule(((BBS4000CardMibBean)v.get(i)).getUtsEponModuleBoardType()))
      {
        v.remove(i);
      }
      else
  ...
  /**
   * remove XGM01A card's useless ports
   * @param portList
   * @param gsmCardList
   * @return
   */
  private Vector filterXgmPort(Vector portList, Vector gsmCardList)
  {
	  ModuleType moduleType = new ModuleType();
	  final int iXGM01A_PORT_VALUE = 1;	// This is the real value of XGM01A card's port;
	  									// remove this card's other useless ports
	  
	  for(int i = 0; i < gsmCardList.size(); i++)
	  {
		  BBS4000CardMibBean cardBean = (BBS4000CardMibBean)gsmCardList.get(i);
		  if( moduleType.isXGMModule( cardBean.getUtsEponModuleBoardType().intValue() ) )
		  {
			  int j = 0;
			  while( j < portList.size() )
			  {
				  Layer2PortTable lptBean = (Layer2PortTable)portList.get(j);
				  if( lptBean.getUtsEthIfExtModuleId().equals( cardBean.getUtsEponModuleBoardPhyId() ) 
						  && iXGM01A_PORT_VALUE != lptBean.getUtsEthIfExtPortId().intValue()
						   )
				  {
					  portList.remove(j);
				  }

/**
 * Created by Zhou Chao, 2010/5/14
 */
...
public class ResetForceMultiCardAction extends SnmpAction {

    public ResetForceMultiCardAction(IApplication anApplication) {
        super(anApplication);
    }
    ...
    public void executeAction(ActionEvent e) throws OperationFailedException, OperationCanceledException, OperationFinishedWithoutLogException {
        ...
        int confirmResult = JOptionPane.showConfirmDialog(fApplication.getTopMostFrame(), warningMessage, warningTitle, JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
        ...
        for (int i = 0; i < selectedRows.length; i++) {
            ...
            // Reset force the card.
            //Task t = new ResetForceMultiCardAction.ResetCardTask(onu);
            Task t = new ResetForceMultiCardAction.ResetForceCardTask(onu);
    ...
    */
    ...
    class ResetForceCardTask extends Task {

        EponModuleBoardSummaryTable card = null;

        public ResetForceCardTask(EponModuleBoardSummaryTable table) {
            card = table;
        }

        public Object execute() throws Exception {
            return card.resetForce() ? Boolean.TRUE : Boolean.FALSE;
        }

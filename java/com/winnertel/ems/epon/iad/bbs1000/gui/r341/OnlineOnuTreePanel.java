...
import com.winnertel.ems.epon.iad.bbs1000.mib.EponModuleBoardTable;
...
public class OnlineOnuTreePanel extends JPanel implements MouseListener {
	...
	int ratio = 32;
	boolean init = false;
	Dot3Onu2CtcSerialNumberFormatter formatter = new Dot3Onu2CtcSerialNumberFormatter(
			fApplication);
	...
	public OnlineOnuTreePanel(IApplication app) {
		fApplication = app;
		root = new CheckTreeNode(fApplication.getActiveDeviceManager()
				.getDisplayName());
	...
	private void loadOnlineOnu() {
		...
		Dot3Onu2CtcTable meanBean = new Dot3Onu2CtcTable(fApplication
				.getSnmpProxy());
		Task t = new RefreshAllTask(meanBean);
		...
		try {
			mibBeanVector = (Vector<Dot3Onu2CtcTable>) fApplication
					.getTaskManager().executeTask(t);
		} catch (Exception ex) {
	...
	private void buildOnlineOnu(Dot3Onu2CtcTable dot3Onu2CtcTable) {
		...
		String onuName = null;
		try {
			onuName = (String) formatter.format(dot3Onu2CtcTable,
					"utsDot3Onu2CtcOnuModel");
		} catch (Exception e) {
	...
	public void loadOnuNode() {
		...
		EponModuleBoardTable bean = new EponModuleBoardTable(fApplication
				.getSnmpProxy());
		Vector v = BeanService.refreshTableBean(fApplication, bean);
		ModuleType moduleType = new ModuleType();
		for (int i = 0; i < v.size(); i++) {
			EponModuleBoardTable one = (EponModuleBoardTable) v.get(i);
			int moduleBoardType = one.getUtsEponModuleBoardType().intValue();
			int moduleBoardState = one.getUtsEponModuleBoardState()
					.intValue();
			if (!(moduleType.isGSMModule(moduleBoardType)) && moduleBoardState == 3) {
				CheckTreeNode treeNode = createCardNode(one
						.getUtsEponModuleBoardPhyId().intValue());
				if (treeNode != null) {
					root.add(treeNode);
				}
			}
				...
				if (portMap.containsKey(i)) {
					...
					for (int j = 1; j <= ratio; j++) {
						if (onuMap.containsKey(j)) {
							...
							stringBuilder.append(")");
							port
									.add(new CheckTreeNode(stringBuilder
											.toString()));
						}
					}
	...
	public byte[] getSelectedOnuBitMap() {
		...
		for (int i = 0; i < v.size(); i++) {
			...
			int index = (module - 1) * cardByteSize + (port - 1) * portByteSize
					+ (onu - 1) / 8;
			int n = (onu - 1) % 8;
	...
	public void mousePressed(MouseEvent e) {
		CheckBoxNodeRender celleditor = (CheckBoxNodeRender) jTree
				.getCellRenderer();
		boolean isInCheckBoxBound = celleditor.isInCellCheckBoxBound(jTree, e
				.getX(), e.getY());
		if (e.getClickCount() == 1 && isInCheckBoxBound) {
			...
			if (path != null) {
				CheckTreeNode node = (CheckTreeNode) path

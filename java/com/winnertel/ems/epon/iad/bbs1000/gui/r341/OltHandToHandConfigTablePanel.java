...
import com.winnertel.ems.epon.iad.bbs1000.mib.r341.OltHandToHandConfigTable;
...
//	private IntegerTextField tfUtsOltHandToHandConfigPortId = new IntegerTextField();
	...
	private void refreshData() {
	    ...
	    try {
	      EponModuleBoardTable moduleMBean = new EponModuleBoardTable(fApplication.getSnmpProxy());

	      Vector v = BeanService.refreshTableBean(fApplication, moduleMBean);

	      ModuleType moduleType = new ModuleType();
	      for (int i = 0; i < v.size(); i++) {
	        int moduleBoardPhyId = ((EponModuleBoardTable) v.get(i)).getUtsEponModuleBoardPhyId().intValue();
	        int moduleBoardType = ((EponModuleBoardTable) v.get(i)).getUtsEponModuleBoardType().intValue();
	        int moduleBoardState = ((EponModuleBoardTable) v.get(i)).getUtsEponModuleBoardState().intValue();
...
//	        } 
	...
	private Vector getPortList() {
        ...
        ModuleType moduleType = new ModuleType();
        for (int i = 0; i < v.size(); i++) {
            int moduleBoardPhyId = ((EponModuleBoardTable) v.get(i)).getUtsEponModuleBoardPhyId();
            int moduleBoardType = ((EponModuleBoardTable) v.get(i)).getUtsEponModuleBoardType();
            int moduleBoardState = ((EponModuleBoardTable) v.get(i)).getUtsEponModuleBoardState();

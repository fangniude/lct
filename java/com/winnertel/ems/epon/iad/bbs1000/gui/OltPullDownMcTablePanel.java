...
public class OltPullDownMcTablePanel extends UPanel {
  ...
  public boolean validateFrom() {
    ...
    OltPullDownMcTable mbean = new OltPullDownMcTable(fApplication.getSnmpProxy());
    Vector v = BeanService.refreshTableBean(fApplication,mbean);
    if(v!=null){
    	for(int i=0;i<v.size();i++){
    		OltPullDownMcTable one = (OltPullDownMcTable)v.get(i);
    		if(tfUtsOltPullDownMcGroupIpAddr.getIPString().equals(one.getUtsOltPullDownMcGroupIpAddr())){
    			String err = fStringMap.getString("Err_Record_Exists");

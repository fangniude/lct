...
//    baseInfoPanel.add(new HSpacer());
  ...
  public boolean validateFrom() {
    ...
    for(int i = 0; i < v.size(); i++){
      bean = (OnuCtcMulticastCtrlTable)v.get(i);
      if(tfUtsDot3OnuModuleId.getSelectedItem().equals(bean.getUtsDot3OnuModuleId().toString())&&tfUtsDot3OnuPortId.getSelectedItem().equals(bean.getUtsDot3OnuPortId().toString())&&
        tfUtsDot3OnuLogicalPortId.getSelectedItem().equals(bean.getUtsDot3OnuLogicalPortId().toString())&&tfUtsDot3OnuEtherPortIndex.getSelectedItem().equals(bean.getUtsDot3OnuEtherPortIndex().toString())&&
        tfUtsDot3OnuMCastVlan.getValue()==bean.getUtsDot3OnuMCastVlan().intValue()&&tfUtsDot3OnuMCastGroupStart.getIPString().equals(bean.getUtsDot3OnuMCastGroupStart())) 
      {
        String msg = fStringMap.getString("Err_Record_Exists");

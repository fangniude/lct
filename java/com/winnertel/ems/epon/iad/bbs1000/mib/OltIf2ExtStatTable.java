...
public class OltIf2ExtStatTable extends SnmpMibBean {
  ...
  public String getIf2Index(){
    return getIndex(0) + "/" + getIndex(2);
  }
  ...
  public Vector retrieveAll() throws MibBeanException {
    ...
    int i = 0;
    while(i < v.size()) {
        OltIf2ExtStatTable one = (OltIf2ExtStatTable)v.get(i);
        
        //remove the onu from the vector.
        if(one.getUtsPonIf2ExtStatLogicalLinkId() != 0) {
            v.remove(i);
        } else {
            i++;
        }

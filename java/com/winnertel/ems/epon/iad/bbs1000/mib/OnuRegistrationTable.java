 ...
 * Created by QuickDVM
 */
public class OnuRegistrationTable extends SnmpMibBean {
...
//  public static final String utsDot3OltOlpPort = "utsDot3OltOlpPort";
  ...
  public OnuRegistrationTable(ISnmpProxy aSnmpProxy) {
    super(aSnmpProxy);
    init();
  }
...
//    prepareRead(super.getProperty(utsDot3OltOlpPort));
    ...
    //remove the port recorder which is offline.
    try
    {
      Vector list = new Vector(2);
      EponModuleSummaryTable moduleBean = new EponModuleSummaryTable(fSnmpProxy);
      Vector cardList = moduleBean.retrieveAll();

      for(int i = 0; i < cardList.size(); i++)
      {
        EponModuleSummaryTable card = (EponModuleSummaryTable)cardList.get(i);

        if(card.getUtsEponModuleBoardState() == 3)//active
        {
          list.add(card.getUtsEponModuleBoardPhyId());
        }
      }
      ...
      while(i < v.size())
      {
        OnuRegistrationTable bean = (OnuRegistrationTable)v.get(i);

        if(!list.contains(bean.getUtsDot3OltModuleId()))
        {
          v.remove(i);
        }
        else

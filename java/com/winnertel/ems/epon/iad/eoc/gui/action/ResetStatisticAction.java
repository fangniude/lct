...
import com.winnertel.ems.epon.iad.eoc.mib.ModEoCRealTimeStatisticTableA;
 ...
 */
public class ResetStatisticAction extends SnmpAction {

    public ResetStatisticAction(IApplication anApplication) {
        super(anApplication);
        putValue(BaseAction.ID, "StatisticReset");
    }
    ...
    public void executeAction(ActionEvent e) throws Exception {
        ...
        if (!MessageDialog.showConfirmOperationDialog(fApplication)) {
            throw new OperationCanceledException("Reset Statistic action canceled");
        }

        // Reset the ONU, set utsDot3Onu2CtcReset 1: reset(1)
        Task t = new ResetStatisticTask(fTable);
        ...
        try {
            ...
            if (!result.booleanValue()) {
                throw new OperationFailedException("Reset Statistic operation failed");
            }
    ...
    class ResetStatisticTask extends Task {

        SnmpTable fTable = null;

        public ResetStatisticTask(SnmpTable table) {
            fTable = table;
        }

        public Object execute() throws Exception {
            SnmpTableModel tableModel = (SnmpTableModel) fTable.getModel();
            Object obj = tableModel.getRow(fTable.getSelectedRow());
            if (obj instanceof ModEoCRealTimeStatisticTable) {
                ModEoCRealTimeStatisticTable bean = (ModEoCRealTimeStatisticTable) obj;
                return bean.resetStatistic() ? Boolean.TRUE : Boolean.FALSE;
            } else if (obj instanceof ModEoCRealTimeStatisticTableA) {
                ModEoCRealTimeStatisticTableA bean = (ModEoCRealTimeStatisticTableA) obj;
                return bean.resetStatistic() ? Boolean.TRUE : Boolean.FALSE;
            } else {
